...
def create_optimizer(model, config):
    optimizer_params = [
        ...
        {
            "params": [p for n, p in model.image_model.named_parameters() if p.requires_grad],
            "lr": config.IMAGE_LR,
            "weight_decay": config.WEIGHT_DECAY,
        },
        {
            "params": [p for n, p in model.regressor.named_parameters() if p.requires_grad],
...
def train(model, train_loader, val_loader, config):
    ...
    for epoch in range(config.EPOCHS):
        ...
        train_loss, train_mae = train_epoch(
            model, train_loader, criterion, optimizer, scheduler, config.device, config
        )
...
def plot_training_history(history):
    ...
    print(f"График сохранен в plots/training_history.png")